...
@Tag(name = "Catálogo: Productos", description = "Operaciones relacionadas con la gestión de productos")
public class ProductoController {
    ...
    // Estos metodos harán referencia a los servicios del dominio, que a su vez
    // interactuarán con los repositorios para obtener o modificar datos en la base
    // de datos.
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Producto creado exitosamente", headers = @Header(name = "Location", description = "URL para acceder al producto creado", schema = @Schema(type = "string")), content = @Content(schema = @Schema(implementation = ProductoResponse.class))),
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Producto encontrado exitosamente", headers = @Header(name = "Location", description = "URL para acceder al producto", schema = @Schema(type = "string")), content = @Content(schema = @Schema(implementation = ProductoResponse.class))),
    ...
    /*
     * En esl siguiente metodo utilizamos @pathvariable para tomar el valor embebido
     * de la url
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Productos obtenidos exitosamente", headers = @Header(name = "Location", description = "URL para acceder a los productos", schema = @Schema(type = "string")), content = @Content(schema = @Schema(implementation = ProductoResponse.class)))
    })
    /*
     * En el siguiente metodo se obtienen todos los productos de la base de datos,
     * es por eso
     * que no tiene una ruta especifica (porque tener ../productos/productos es
     * redundante)
     */
    /*
     * En el siguiente metodo se obtienen todos los productos de la base de datos,
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Producto actualizado exitosamente", headers = @Header(name = "Location", description = "URL para acceder al producto actualizado", schema = @Schema(type = "string")), content = @Content(schema = @Schema(implementation = ProductoResponse.class))),
            @ApiResponse(responseCode = "400", description = "Datos de entrada inválidos", content = @Content(schema = @Schema(implementation = ApiError.class))),
            @ApiResponse(responseCode = "404", description = "Producto no encontrado", content = @Content(schema = @Schema(implementation = ApiError.class)))
    })
    /*
     * En el siguiente metodo utilizamos @pathvariable para tomar el valor embebido
     * de la url
     */
    // Ahora añadiremos un endpoint para actualizar un producto, utilizando PUT y
    // recibiendo el ID del producto a actualizar en la URL, y los nuevos datos en
    // el cuerpo de la solicitud.
    @PutMapping("/{id}")
    public ResponseEntity<ProductoResponse> actualizarProducto(@PathVariable UUID id,
